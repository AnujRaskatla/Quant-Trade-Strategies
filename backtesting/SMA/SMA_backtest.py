...
buy_price = 0  # Price at which the shares were bought

for i in range(len(Data)):
    if not np.isnan(Data['b'][i]):  # Buy signal
        position = balance / Data['SMA30'][i]
        buy_price = Data['SMA30'][i]
        balance = 0
    elif not np.isnan(Data['s'][i]):  # Sell signal
        balance = position * Data['SMA30'][i]
        position = 0

# If still holding shares at the end, liquidate the position
balance += position * Data['SMA30'].iloc[-1]
